...
export function formatInTz(date: Date | number, tz: string, pattern: string): string {
...
export function isTodayInTz(date: Date | number, tz: string, now: Date = new Date()): boolean {
  return isSameDayInTz(date, now, tz);
}

export function isTomorrowInTz(date: Date | number, tz: string, now: Date = new Date()): boolean {
...
export function fromLocalInputValue(value: string, tz: string): Date | null {
  ...
  if (!m) return null;
  const [, y, mo, d, h, mi] = m;
  const zoned = new TZDate(Number(y), Number(mo) - 1, Number(d), Number(h), Number(mi), tz);
  const t = zoned.getTime();
  return Number.isNaN(t) ? null : new Date(t);
}

/** Same calendar day as `date` (in `tz`) at the given wall-clock time. */
export function atTimeInTz(date: Date | number, tz: string, hour: number, minute = 0): Date {
  const z = inTz(date, tz);
  return new Date(new TZDate(z.getFullYear(), z.getMonth(), z.getDate(), hour, minute, tz).getTime());
}
...
export function formatTime(date: Date | number, tz: string, hour12: boolean = clockHour12()): string {
...
/** "Wed 17 Sep · 14:05" */
export function formatDateTime(date: Date | number, tz: string, hour12: boolean = clockHour12()): string {
...
export function formatWhen(
  ...
  return isTodayInTz(date, tz, now) ? formatTime(date, tz, hour12) : formatDateTime(date, tz, hour12);
}
 ...
 */
export function formatClock(hhmm: string, hour12: boolean = clockHour12()): string {
 ...
 */
export function formatHour(hour: number, hour12: boolean = clockHour12()): string {
...
export function relativeToNow(date: Date | number, now: Date = new Date()): string {
