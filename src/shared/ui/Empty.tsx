...
export function Empty({ icon, title, body, action }: { icon?: ReactNode; title: string; body?: ReactNode; action?: ReactNode }) {
  return (
    <div className="mx-3 flex flex-col items-center gap-2 rounded-2xl border border-dashed border-rule px-5 py-7 text-center">
      {icon ? <span className="flex h-11 w-11 items-center justify-center rounded-full bg-accent-soft text-accent">{icon}</span> : null}
...
export function Placeholder({ icon, title, body, action }: { icon?: ReactNode; title: string; body?: ReactNode; action?: ReactNode }) {
  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
      {icon ? <span className="flex h-14 w-14 items-center justify-center rounded-full bg-accent-soft text-accent">{icon}</span> : null}
