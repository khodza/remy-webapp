...
export function ImportPage() {
  ...
  useMainButton(
    drafts === null
      ? {
          text: parse.isPending ? 'Reading…' : 'Read the list',
          enabled: lines > 0 && lines <= MAX_LINES && !parse.isPending,
    ...
    return (
      <Screen>
        ...
        <p className="px-4 pb-3 text-[13.5px] font-semibold text-muted">
          Paste a to-do list from Notes or anywhere else, one thing per line. Remy reads the times; lines without one go
          to the Inbox. You check everything before it's added.
        </p>
        <Group>
          <AutoTextarea
            value={text}
        ...
        <p className={`tnum px-4 pt-2 text-[12.5px] font-bold ${lines > MAX_LINES ? 'text-danger' : 'text-muted'}`}>
          {lines === 0
            ...
            : `${lines} ${lines === 1 ? 'line' : 'lines'}${lines > MAX_LINES ? ` · at most ${MAX_LINES} at a time` : ''}`}
        </p>
      </Screen>
    );
  }
    ...
    <Screen>
      ...
      {pastCount > 0 ? (
        <p className="px-4 pt-2 text-[12.5px] font-bold text-danger">
          {pastCount === 1 ? 'One time has' : `${pastCount} times have`} already passed. Tap it to pick a new one, or
          leave the line out.
        </p>
      ) : null}
      <p className="px-4 pt-2 text-[12.5px] font-semibold text-muted">
        Tap a circle to leave a line out, or a time to change it.
      </p>
