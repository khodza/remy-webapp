  ...
  return (
    <Screen>
      ...
      <Group>
        <FieldRow
          icon={<Moon size={16} />}
          label="Hold reminders at night"
          trailing={
            <Toggle checked={q.enabled} onChange={(enabled) => save({ quietHours: { enabled } })} label="Quiet hours" />
          }
        />
        <FieldRow
          label="From"
        ...
        <FieldRow
          ...
          trailing={
            <Toggle
              checked={q.allowHighPriority}
              disabled={!q.enabled}
              onChange={(allowHighPriority) => save({ quietHours: { allowHighPriority } })}
              label="High priority rings during quiet hours"
            />
          }
        />
      </Group>
      <p className="px-4 pt-2 text-[12.5px] font-semibold text-muted">
        Reminders due in this window arrive when it ends.
      </p>
        ...
        <FieldRow
          ...
          trailing={
            <Toggle
              ...
              label="Nudge if ignored"
            />
          }
        />
        <FieldRow
          label="First nudge"
          value={`after ${minutesLabel(first)}`}
          {...(s.escalation.enabled ? { onClick: () => setPicking('first') } : {})}
        />
        <FieldRow
          label="Second nudge"
          value={second ? `after ${minutesLabel(second)}` : 'None'}
          {...(s.escalation.enabled ? { onClick: () => setPicking('second') } : {})}
        />
      </Group>
      <p className="px-4 pt-2 text-[12.5px] font-semibold text-muted">
        Counted from the reminder. Low-priority reminders are never nudged.
      </p>
