...
describe('feedLinks', () => {
  it('makes an absolute link from an absolute API base', () => {
    ...
    expect(links).toEqual({
      ...
      google:
        'https://calendar.google.com/calendar/render?cid=webcal%3A%2F%2Fapi.remy.dev%2Fapi%2Fv1%2Fcalendar%2Fabc.ics',
      local: false,
    });
  });

  it('uses the page origin behind the dev proxy and flags local hosts', () => {
    expect(feedLinks('/api/v1/calendar/abc.ics', 'https://x.ngrok-free.dev').https).toBe(
      'https://x.ngrok-free.dev/api/v1/calendar/abc.ics',
    );
